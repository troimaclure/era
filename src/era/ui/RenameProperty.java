 ...
 */
public class RenameProperty extends javax.swing.JFrame {

    public int x = 0;
    public int y = 0;

    public RenameProperty(Property prop) {
        initComponents();
        type.setText(prop.type);
        name.setText(prop.name);
        comment.setText(prop.comment);
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        EntiteManager.getEntites().stream().filter(e -> e.selected).forEach((entite) -> {
            entite.getProps().stream().filter((d) -> d.selected).forEach((d) -> {
                d.name = this.name.getText();
                d.comment = this.comment.getText();
                d.type = this.type.getText();
            });
        });
        this.setVisible(false);
